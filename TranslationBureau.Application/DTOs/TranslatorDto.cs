...
public class TranslatorDto
{
    ...
    /// <summary>Коды языков переводчика (для отображения в списке).</summary>
    public IReadOnlyList<string> LanguageCodes { get; init; } = [];

    /// <summary>Наименования языков (для отображения в списке).</summary>
    public IReadOnlyList<string> LanguageNames { get; init; } = [];
}